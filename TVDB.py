...
class TVShow:
    ...
    def get_episode(self):
        r = requests.get("http://thetvdb.com/api/{0}/series/{1}/default/{2}/{3}/{4}.xml"
            .format(API_KEY, self.series, str(self.season).lstrip("0"), str(self.episode).lstrip("0"), LANGUAGE))
    ...
    if os.path.isdir(directory):
        ...
        for file_ in os.listdir(directory):
            ...
            if split[-1].upper() in FILE_TYPES:
                ...
                r.rename(os.sep.join([directory, file_]), str(tvshow), extension=split[-1], extensions=FILE_TYPES)
