...
def rename(file_, new_name, extension="", extensions=[]):
	if not os.path.exists(file_):
		...
		file_name = os.extsep.join(file_.split(os.sep)[-1].split(os.extsep)[0:-1])
		extension = file_.split(os.extsep)[-1]

		# Check if the file will actually be a different filename, only rename if it will
		if file_name == new_name:
			output = "No change: " + new_name

		else: 
			os.rename(file_, os.path.join(directory, os.extsep.join([new_name, extension])))
		...
		# print all but that word, otherwise print the full word (incase there isn't an extension)
			output = (file_name if extension_check(extension, extensions) \
				else os.extsep.join([file_name, extension])) + " -> " + new_name
